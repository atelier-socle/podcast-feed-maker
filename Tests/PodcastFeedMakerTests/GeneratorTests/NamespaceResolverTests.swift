...
struct NamespaceResolverTests {
    ...
    func detectsPodcastFromItem() {
        var ch = minimalChannel()
        ch.items = [Item(transcripts: [Transcript(url: URL(string: "https://example.com/t.vtt")!, type: "text/vtt")])]
        let resolved = NamespaceResolver.resolve(feedWith(channel: ch))
        #expect(resolved.contains(.podcast))
    }
    ...
    func detectsPodcastFromVariousChannelProps() {
        ...
        ch2.funding = [Funding(url: URL(string: "https://example.com")!, message: "Support")]
        ...
        ch4.trailers = [Trailer(title: "T", url: URL(string: "https://example.com/t.mp3")!, pubDate: Date())]
